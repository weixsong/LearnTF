...
def BiRNN(x, weights, biases):
    ...
    # method 2: by gather
    batch_size, step_size = tf.shape(outputs)[0], tf.shape(outputs)[1]
    # outputs = tf.reshape(outputs, [-1, n_hidden * 2])
    index = tf.range(0, batch_size) * step_size + (step_size - 1)
    print("outputs shape after transpose: %s" % str(outputs.get_shape()))
    outputs = tf.gather(tf.reshape(outputs, [-1, n_hidden * 2]), index)
    print("outputs shape after gather: %s" % str(outputs.get_shape()))
